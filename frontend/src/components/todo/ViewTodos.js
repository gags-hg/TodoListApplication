...
function Todos({isAuthenticated, setIsAuthenticated}) {
	...
	return (
		<Base>
		<Container>
			<Row className="mt-4"> <Col>  <Card    color="light" > 
			< div className="container">
			<CardHeader>
			<h1 className="text-center"><u>Todo List</u></h1>
			
			<div>{isAuthenticated && <Link to='/add'><button className="btn btn-danger" style={{margin:"5px 0px"}}>ADD TODO</button></Link>}</div>
			{showErrorMessage()}
        ...
        <tbody>
        {
          todos.map((todo) => {
            return <tr className={todo.isCompleted? 'completed' : ''} key={todo.id} >
			  ...
			  checked={todo.isCompleted} onClick={() => markCompleted(todo.id)}/></td>
              ...
              <td>{todo.isCompleted.toString()}</td>
			  				{/* <td><div><input class="form-check-input" type="checkbox" value="" id="flexCheckDefault" onClick={() => markCompleted(todo.id)}/></div></td> */}
			...
			<Row style={{marginBottom:'2rem', marginTop:'1rem', marginLeft:'4rem'}}>
